...
class EquipmentImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        // Check if required fields are present; return null if any are missing
        if (is_null($data['unit_no']) || is_null($data['description']) || is_null($data['facility_id']) || is_null($data['category_id']) || is_null($data['stock_unit'])) {
